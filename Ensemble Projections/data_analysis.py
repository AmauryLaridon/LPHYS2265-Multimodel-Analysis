...
def subplot_TSIMAL_SIGUS_ENS():
    gs = gridspec.GridSpec(2, 2, wspace=0.25, hspace=0.3, left=0.1, right=0.98)
    plt.figure()
    plt.suptitle(
        r"Ensemble model and TISMAL\&SIGUS output $(h_i, T_{su}, h_s)$ for the CTL simulation"
    )
    ### Figure 1 ###
    ax = plt.subplot(gs[0, 0])  # row 0, col 0
    plt.plot(time_range_ctl, hi[:, 13], linewidth=1, label="TSIMAL", color="red")
    plt.plot(time_range_ctl, hi[:, 11], linewidth=1, label="SIGUS", color="orange")
    mod_mean = mean_all_mod(data=hi, N=N_days_CTL, N_mod=N_mod_CTL)
    plt.plot(
        time_range_ctl, mod_mean, linewidth=2, color="tab:blue", label=r"Models Mean"
    )
    plt.xlabel("Days", size=10)
    ...
    ### Figure 2 ###
    ax = plt.subplot(gs[0, 1])  # row 0, col 1
    plt.plot(time_range_ctl, Tsu[:, 13], linewidth=1, label="TSIMAL", color="red")
    plt.plot(time_range_ctl, Tsu[:, 11], linewidth=1, label="SIGUS", color="orange")
    mod_mean = mean_all_mod(data=Tsu, N=N_days_CTL, N_mod=N_mod_CTL)
    plt.plot(
        time_range_ctl, mod_mean, linewidth=2, color="tab:blue", label=r"Models Mean"
    )
    plt.xlabel("Days", size=10)
    ...
    ### Figure 3 ###
    ax = plt.subplot(gs[1, :])  # row 1, span all columns
    plt.plot(time_range_ctl, hs[:, 13], linewidth=1, label="TSIMAL", color="red")
    plt.plot(time_range_ctl, hs[:, 11], linewidth=1, label="SIGUS", color="orange")
    mod_mean = mean_all_mod(data=hs, N=N_days_CTL, N_mod=N_mod_CTL)
    plt.plot(
        time_range_ctl, mod_mean, linewidth=2, color="tab:blue", label=r"Models Mean"
    )
    plt.xlabel("Days", size=10)
...
""" if __name__ == "__main__":
    ...
    plot_all_mod(data=hi_mean_month, data_name="hi_mean_month", N_mod=N_mod_CTL)
    subplot_all_mod(
        data1=hi_mean_month,
        data2=Tsu,
        data3=hs,

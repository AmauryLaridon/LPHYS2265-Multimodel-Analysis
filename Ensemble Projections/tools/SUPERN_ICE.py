...
# model parameters
gam_SM      = 1.065        # Conductivity Semtner factor
...
for day in range(1,Nd):
   ...
   #--------------------
      ...
      if Tw[day] <= Tb:
         ...
         h_i[day]   = h_i[day-1] + dh_ow                             # update thickness
...
#np.savetxt('CTL_SUPERN_ICE.txt', data, delimiter=" ", fmt='%s')
...
np.savetxt('PR12_SUPERN_ICE.txt', data, delimiter=" ", fmt='%s')
